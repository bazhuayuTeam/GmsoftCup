...
public class StringTools
{
	...
	//数字转换为百分数并保留两位小数
	public static String getPercentage(Object number){
		double newNumbers = Double.parseDouble(number.toString());
		DecimalFormat df = new DecimalFormat("#.00");
		String test = df.format((newNumbers*100))+"%";
		if('.'==test.charAt(0)){
			return "0"+df.format((newNumbers*100))+"%";
		}else{
			return df.format((newNumbers*100))+"%";
		}
	}
}
