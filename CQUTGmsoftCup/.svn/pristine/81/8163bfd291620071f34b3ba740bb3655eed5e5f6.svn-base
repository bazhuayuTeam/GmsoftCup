...
(function($){
	...
	_setting = {
		...
		isLeaf:function(node){return false;},//判断节点是否是叶节点
	 	dataProxys:{},//所有数据加载代理
	 	dataProxySelecter:null,//数据选择器
	 	loadConditionFun:null,//加载前条件处理
...
//					if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle)) {html.push("title='", node[titleKey].replace(/'/g,"&#39;"),"'");}
					//二次开发代码-----------------------------------
					if (tools.apply(setting.view.showTitle, [setting.treeId, node], setting.view.showTitle)) {html.push("title='", ((_curTitle = (node[titleKey] ? node[titleKey] : node[node.nameCol]))?_curTitle:'').replace(/'/g,"&#39;"),"'");}
					html.push(">");
					...
					html.push("<button type='button' id='", node.tId, consts.id.ICON,
						...
						"'>",((_curKey = (node[nameKey] ? node[nameKey] : node[node.nameCol])) ? _curKey : '').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),"</span>");
					//二次开发代码-----------------------------------
//						"'>",node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),"</span>");
		...
		asyncNode: function(setting, node, isSilent, callback) {
			...
			if(setting.dwr){
				...
				if (!tools.isArray(proxyNames)) {//不是一个数组
					...
					proxyNames = $.unique($(proxyNames));//去掉重复的
					proxyNames = $.unique($(proxyNames));//执行次方法后顺序会倒置，所有执行两次
				}
				...
				$(proxyNames).each(function(index, curProxyName){
					...
					view.dwrDataLoader(proxy, curProxyName,setting, node, isSilent, callback);
				});
				//二次开发代码------------------------------------------------------------------------------
			}else{
				$.ajax({
		...
		//二次开发代码------------------------------------------------------------------------------
		dwrDataLoader: function(proxy, curProxyName, setting, node, isSilent, callback){
			if(proxy!=undefined && proxy!=null && proxy.dwrFun){
				...
				if(setting.loadConditionFun){
					var userCondition = setting.loadConditionFun(node, curProxyName);
				...
				proxy.dwrFun(proxy.properties, condition, 
					proxy.sortField, proxy.order, proxy.needLink, 
					function(msg) {
						//组织节点数据
						var newNodes = [];
						try {
						...
						for (i = 0; i < newNodes.length; i++) {
							...
							newNodes[i].nameCol = proxy.nameCol;//显示用的属性
							newNodes[i].titleKey = proxy.treeNodeKey;//titleKey
		...
		setNodeName: function(setting, node) {
			...
			//二次开发代码------------------------------------
			//用于显示
			if(setting.dwr && node.nameCol){
				nameKey = node.nameCol;
			}
			//二次开发代码------------------------------------
