...
KE.selection = function(doc) {
	...
	this.init = function() {
		...
		if (KE.browser.IE) {
			...
			if (startNode && startNode.nodeType == 1 && typeof startNode.childNodes[startPos] != 'undefined') {
...
//*************************对于火狐的延迟加载就是、不是很清楚 采用了一下的方式处理---改（zcr）
KE.Ghtml = function(id, val) {
	if (val === undefined) {
		return KE.util.getData(id);
	} else {
		KE.g[id].statusID = setInterval("KE.htmlF('"+id+"','"+escape(val)+"')",100);
	}
};

KE.htmlF = function(id, val) {
	if (val === undefined) {
		return KE.util.getData(id);
	} else {
		if(!KE.g[id].container){
			...
			KE.html(id,unescape(val));
			clearInterval(KE.g[id].statusID);
		}
	}
};
//*************************对于火狐的延迟加载就是、不是很清楚 采用了一下的方式处理---改（zcr）end
