...
*/
		...
		dialog_box.dialogNew('content', that.length ? that[0] : '<div>' + that.selector + '</div>');
		...
		window.top.$('body').append(dialog_box);

		if (options.shown) {
			dialog_box.dialogNew('show');
		}
		if (!options.modal) {
			dialog_box.dialogNew('resize');
		}
